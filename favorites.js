// favorites.js – полная копия recipes.js, но с endpoint для избранных
...
// ========== Фильтрация и загрузка избранных рецептов ==========
...
async function loadRecipesWithFilters() {
    ...
    // !!! ИЗМЕНЕНИЕ: используем endpoint для избранных рецептов
    const url = `${API_BASE}/api/recipes/favorites?${urlParams.toString()}`;
    console.log('Запрос избранных рецептов:', url);
    try {
        const response = await fetch(url);
        if (!response.ok) throw new Error('Ошибка загрузки избранных рецептов');
        ...
        console.error('Не удалось загрузить избранные рецепты:', error);
        const container = document.querySelector('.catalog .container-cards');
        if (container) container.innerHTML = '<p class="text-normal">Не удалось загрузить избранные рецепты</p>';
    }
}
...
function renderRecipes(recipes) {
    ...
    if (recipes.length === 0) {
        container.innerHTML = '<p class="text-normal">Избранных рецептов нет</p>';
    } else {
...
// ========== Избранное (переключение на карточках) ==========
document.querySelector('.catalog').addEventListener('click', async (e) => {
    ...
    try {
        ...
        // Если мы на странице избранного и убрали лайк — можно перезагрузить список
        loadRecipesWithFilters();
    } catch (error) {
